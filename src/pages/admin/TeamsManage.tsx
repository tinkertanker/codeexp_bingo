...
import type { Team, TeamColour, TeamId } from '../../lib/types'
...
function Manage({ mentorName, passcode }: { mentorName: string; passcode: string }) {
  ...
  const sortedTeams = teams
    ? [...teams].sort((a, b) => a.colour.localeCompare(b.colour) || a.name.localeCompare(b.name))
    : []
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await createTeam({ passcode, mentorName, name: newName.trim(), colour: newColour })
      setNewName('')
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <ul className="space-y-2">
          {sortedTeams.map((t) => (
            <li key={t._id} className="bh-card p-3 flex items-center gap-3">
